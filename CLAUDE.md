...
- [ ] **Gender DIF omnibus vignette** — Omnibus DIF/measurement invariance by gender across all IRW datasets with `cov_gender` or `cov_sex`. Compute script at `vignettes/gender_dif_compute.R`; prototype on 10 datasets first. In progress on branch `vignette_dev`.
- [ ] **PISA DIF replication** — Replicate Ouyang et al. (JASA 2026) L1-sparsity DIF method using IRW PISA 2022 math data (37 OECD countries, 169 items, 277k students). Script at `vignettes/pisa_dif_replication/code/01_estimation_math.R`. In progress on branch `vignette_dev`.
- [ ] **Enhanced IMV vignette** — Extend `vignettes/imv.qmd` to recreate evidence from the Domingue IMV Psychometrika paper: multi-dataset comparisons, IMV distributions, relationship to other fit stats.
- [ ] **Polytomous model comparison via IMV** — Compare binomial IRT (varied link functions: logit, probit, CLL, log-log, cauchit) vs. full polytomous IRT (GRM, GPCM) across IRW polytomous datasets using IMV as the metric.
- [ ] **Rater IRT models** — Apply many-facets Rasch to IRW datasets with a `rater` column. Scope data availability first.
