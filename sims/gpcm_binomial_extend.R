# sims/gpcm_binomial_extend.R
#
# Extends gpcm_binomial_results.rds with K=4 and K=6 conditions.
# Runs 100 reps for each new K, then combines with the existing
# K=5 and K=7 results and overwrites gpcm_binomial_results.rds.
#
# Usage: Rscript sims/gpcm_binomial_extend.R
...
binom_probs <- function(theta, a, b, K, link) {
  eta <- a * theta - b
  p   <- switch(link, logit = plogis(eta), cloglog = 1 - exp(-exp(eta)),
...
fit_gpcm <- function(X, theta) {
  K <- max(X) + 1L
  opt <- optim(c(log(1.5), seq(-1.5, 1.5, len = K - 1L)),
               function(par) -item_ll(gpcm_probs(theta, exp(par[1]), par[-1]), X),
               method = "BFGS", control = list(maxit = 3000, reltol = 1e-9))
  list(a = exp(opt$par[1]), b = opt$par[-1])
}
fit_pcm <- function(X, theta) {
  K <- max(X) + 1L
  opt <- optim(seq(-1.5, 1.5, len = K - 1L),
               function(b) -item_ll(gpcm_probs(theta, 1.0, b), X),
               method = "BFGS", control = list(maxit = 3000, reltol = 1e-9))
...
fit_grm <- function(X, theta) {
  ...
  opt    <- optim(init, function(par) {
    a <- exp(par[1]); b <- if (K == 2L) par[2] else cumsum(c(par[2], exp(par[3:K])))
...
fit_binom_1pl <- function(X, theta, link) {
  ...
  opt <- optim(0, function(b) -item_ll(binom_probs(theta, 1.0, b, K, link), X),
               method = "Brent", lower = -6, upper = 6)
  list(a = 1.0, b = opt$par)
}
fit_scobit <- function(X, theta) {
  K   <- max(X) + 1L
  opt <- optim(c(0, 0), function(par) -item_ll(scobit_probs(theta, par[1], exp(par[2]), K), X),
               method = "BFGS", control = list(maxit = 3000, reltol = 1e-9))
...
fit_binom_2pl <- function(X, theta, link = "logit") {
  ...
  opt <- optim(c(1, 0), function(par) -item_ll(binom_probs(theta, par[1], par[2], K, link), X),
               method = "BFGS")
...
predict_model <- function(nm, f, theta_te, K) {
  switch(nm,
    ...
    np_spline = {
      p_hat <- pmin(pmax(as.numeric(predict(f$mod, newdata = data.frame(th = theta_te),
...
# ── run_one (identical logic to sweep.R) ──────────────────────────────────────
...
run_one <- function(K, asym, rep_idx) {
  ...
  extract_params <- function(mod) lapply(seq_len(N_ITEMS), function(j) {
    p <- coef(mod, IRTpars = TRUE)[[j]]
    list(a = as.numeric(p[, "a"]), b = as.numeric(p[, paste0("b", seq_len(K - 1L))]))
  })

  stacked <- lapply(setNames(model_names, model_names), function(.) matrix(numeric(0), ncol = K))
  for (nm in model_names) {
    fitted <- NULL
    if (nm == "gpcm") {
      fitted <- extract_params(mod_gpcm)
    } else if (nm == "pcm") {
      pv <- suppressMessages(mirt(X_tr_df, 1, itemtype = "gpcm", pars = "values", verbose = FALSE))
      pv$value[pv$name == "a1"] <- 1; pv$est[pv$name == "a1"] <- FALSE
      mod_pcm <- tryCatch(suppressMessages(mirt(X_tr_df, 1, itemtype = "gpcm", pars = pv, verbose = FALSE)),
                          error = function(e) NULL)
      if (!is.null(mod_pcm)) fitted <- extract_params(mod_pcm)
    } else if (nm == "grm") {
      mod_grm <- tryCatch(suppressMessages(mirt(X_tr_df, 1, itemtype = "graded", verbose = FALSE)),
                          error = function(e) NULL)
      if (!is.null(mod_grm)) fitted <- extract_params(mod_grm)
    } else if (nm == "np_spline") {
      fit_list <- lapply(seq_len(N_ITEMS), function(j) {
        obs_j <- !ho_mat[, j]
        df_j  <- data.frame(X = X_mat[obs_j, j], th = theta_eap[obs_j])
        tryCatch(list(mod = scam::scam(cbind(X, K - 1L - X) ~ s(th, bs = "mpi"),
      ...
      fit_list <- lapply(seq_len(N_ITEMS), function(j) {
        obs_j <- !ho_mat[, j]
        tryCatch(fit_model(nm, X_mat[obs_j, j], theta_eap[obs_j]), error = function(e) NULL)
      })
      if (!any(sapply(fit_list, is.null))) fitted <- fit_list
    }
    if (is.null(fitted)) next
    total_ll <- matrix(0, N_PERSONS, length(theta_grid)); ok <- TRUE
    for (j in seq_len(N_ITEMS)) {
      obs_j   <- which(!ho_mat[, j])
      pr_grid <- tryCatch(predict_model(nm, fitted[[j]], theta_grid, K), error = function(e) NULL)
      if (is.null(pr_grid) || anyNA(pr_grid)) { ok <- FALSE; break }
      total_ll[obs_j, ] <- total_ll[obs_j, ] + t(log(pmax(pr_grid, 1e-15)))[X_mat[obs_j, j] + 1L, ]
    }
    if (!ok) next
    ...
    for (j in seq_len(N_ITEMS)) {
      ...
      pr_j <- tryCatch(predict_model(nm, fitted[[j]], theta_est[ho_row[cells_j]], K),
  ...
  pr_1pl <- stacked[["binom_1pl_logit"]]; pr_gpcm <- stacked[["gpcm"]]
  rows <- lapply(model_names, function(nm) {
    ...
    data.frame(K = K, asym = asym, rep = rep_idx, model = nm,
...
# ── Run K=4 and K=6 ───────────────────────────────────────────────────────────
jobs      <- expand.grid(K = K_new, rep = seq_len(N_REPS), stringsAsFactors = FALSE)
jobs$asym <- runif(nrow(jobs), -log(8), log(8))

message(sprintf("Running %d jobs (K in {4,6}, %d reps) on %d cores ...",
                nrow(jobs), N_REPS, N_CORES))

new_results <- mclapply(seq_len(nrow(jobs)), function(i) {
  r <- jobs[i, ]
  tryCatch(run_one(r$K, r$asym, r$rep), error = function(e) {
    message("job ", i, " failed: ", e$message); NULL
  })
}, mc.cores = N_CORES, mc.preschedule = FALSE)

new_results <- do.call(rbind, Filter(Negate(is.null), new_results))
message(sprintf("New K={4,6} results: %d rows", nrow(new_results)))

# ── Combine with existing K=5,7 results and save ──────────────────────────────
existing <- readRDS(existing_file)
combined <- rbind(existing, new_results)
combined$model <- factor(combined$model, levels = model_names)
saveRDS(combined, out_file)
message(sprintf("Combined: %d rows → %s", nrow(combined), out_file))

combined$asym_bin <- cut(combined$asym, breaks = c(-log(8), -0.5, 0.5, log(8)),
                         labels = c("lower-wider", "symmetric", "upper-wider"))
agg <- aggregate(cbind(rmse, imv_c, imv_t) ~ model + K + asym_bin,
                 data = combined, FUN = mean, na.rm = TRUE)
print(agg[order(agg$K, agg$asym_bin, agg$imv_c), ], digits = 4, row.names = FALSE)
