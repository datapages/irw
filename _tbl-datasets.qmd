...
ds4 = await redivis.user('datapages').dataset('item_response_warehouse_4')
ds5 = await redivis.user('datapages').dataset('item_response_warehouse_5')
ds6 = await redivis.user('datapages').dataset('item_response_warehouse_6')

tables1 = await ds1.listTables()
tables2 = await ds2.listTables()
tables3 = await ds3.listTables()
tables4 = await ds4.listTables()
tables5 = await ds5.listTables()
tables6 = await ds6.listTables()
...
table_dataset_map = new Map([
  ...
  ...tables4.map(t => [t.name, ds4]),
  ...tables5.map(t => [t.name, ds5]),
  ...tables6.map(t => [t.name, ds6])
])

dataset_names = [...new Set(
  [...tables1, ...tables2, ...tables3, ...tables4, ...tables5, ...tables6]
    .map(t => t.name)
