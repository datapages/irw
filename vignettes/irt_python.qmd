...
<!-- This vignette is Python-only, so it renders with Quarto's jupyter engine.
     The _source-links.qmd include carries an R chunk, which would flip the file
     to the knitr engine and require reticulate (not in renv.lock) in CI, so the
     link is written out by hand here instead. -->

**Source code for this page:** [`irt_python.qmd`](https://github.com/datapages/irw/blob/main/vignettes/irt_python.qmd)

<!-- Same reason the _data-version.qmd include is not used here: it is an R
     chunk. This page fetches from Redivis at render time, so it has no fixed
     version to state; the pointer to irw.version() is the honest substitute. -->

This page fetches IRW data when the site is built, so it reflects whichever IRW
version was newest at render time. Call `irw.version()` to see which that is,
and `irw.version(date=...)` to recover the version live on any past date.
