...
tran=function(z, x, mu, sigma){
  ...
  sd.vec=var.vec^0.5
  Theta=sqrt(2)*(sd.vec%*%t(rep(1,n)))*(rep(1,N)%*%t(z))+mean.vec%*%t(rep(1,n))
  return(Theta)
}
...
target_function1=function(phi_j, x, Y_j, weights, z, mu_pres, sigma_pres, q){
  ...
  b_j=phi_j; g_j=rep(0,p); a_j=1
  Theta=tran(z, x, mu_pres, sigma_pres)
  temp=Theta*a_j+b_j*(rep(1,N)%*%t(rep(1,n)))+(x%*%g_j)%*%t(rep(1,n))
  t2=Y_j%*%t(rep(1,n))*temp-log(1+exp(temp))
  log_likelihood=(weights%*%t(rep(1,n)))*t2
...
target_function_g1=function(ab, k, gk, gp, x_k, Y_k, weights, z, ind_k, mu_pres, sigma_pres, q){
  N_k=dim(x_k)[1]; p=dim(x_k)[2]; n=length(z)
  a_j=1; b_j=0; gp[k]=gk; g_j=gp
  q_k=q[ind_k,]; weight_k=weights[ind_k]
  Theta=tran(z, x_k, mu_pres, sigma_pres)
  temp=Theta*a_j+b_j*(rep(1,N_k)%*%t(rep(1,n)))+(x_k%*%g_j)%*%t(rep(1,n))
  t2=Y_k%*%t(rep(1,n))*temp-log(1+exp(temp))
  log_likelihood=(weight_k%*%t(rep(1,n)))*t2
...
target_mean=function(para_k, k, x, weights, z, mu_pres, sigma_pres, q){
  N=dim(x)[1]; p=dim(x)[2]; n=length(z)
  me=mu_pres; me[k]=para_k
  Theta=tran(z, x, mu_pres, sigma_pres)
  mean.v=apply(x*(rep(1,N)%*%t(me)), 1, sum, na.rm=TRUE)
  var.v=apply(x*(rep(1,N)%*%t(sigma_pres^2)), 1, sum, na.rm=TRUE)
  sd.v=var.v^0.5
  t1=-0.5*((Theta-mean.v%*%t(rep(1,n)))/(sd.v%*%t(rep(1,n))))^2
  log_likelihood=(weights%*%t(rep(1,n)))*t1
  res=-sum(q*log_likelihood, na.rm=TRUE)
  return(res/N)
}
...
EM_2PL_inference <- function(a, b, g, x, dat, weights, z, mu, sigma, w, ite, tol=0.001){
  N=dim(x)[1]; p=dim(x)[2]; n=length(z); J=dim(dat)[2]
  phi0=matrix(0, J, 2+p); phi1=cbind(a, b, g)
  mu0=rep(0,p); mu1=mu; sigma0=rep(0,p); sigma1=sigma
  ind_group=list(); dat_group=list(); x_group=list()
  for (k in 1:p){
    ind_c=which(x[,k]==1)
    ind_group[[k]]=ind_c; dat_group[[k]]=dat[ind_c,]; x_group[[k]]=x[ind_c,]
  }
  iter <- 0
  while(max(c(abs(phi0-phi1)), abs(mu0-mu1), abs(sigma0-sigma1))>tol){
    ...
    phi1[1,1]=0.4738117; mu1=rep(0,p); phi1[,2]=rep(0,J)
    for(j in 1:J){
      if(j==1){
        for (k in 1:p){
          par_updates_g=optim(par=phi1[j,2+k], fn=target_function_g1, gr=grad_phi_g1, method="L-BFGS-B",
            ab=phi1[j,c(1,2)], k=k, gp=phi1[j,3:(2+p)], x_k=x_group[[k]], Y_k=dat_group[[k]][,j],
            ind_k=ind_group[[k]], weights=weights, z=z, mu_pres=mu1, sigma_pres=sigma1, q=quadrature, lower=-5, upper=5)
          phi1[j,2+k]=par_updates_g$par
        }
      } else {
        for (l in 1:ite){
          par_updates_ab=optim(par=phi1[j,1], fn=target_function_ab, gr=grad_phi_ab, method="L-BFGS-B",
            g=phi1[j,3:(2+p)], x=x, Y_j=dat[,j], weights=weights, z=z, mu_pres=mu0, sigma_pres=sigma0,
            q=quadrature, lower=-5, upper=5)
          phi1[j,1]=par_updates_ab$par
          for (k in 1:p){
            par_updates_g=optim(par=phi1[j,2+k], fn=target_function_g, gr=grad_phi_g, method="L-BFGS-B",
              ab=phi1[j,c(1,2)], k=k, gp=phi1[j,3:(2+p)], x_k=x_group[[k]], Y_k=dat_group[[k]][,j],
              weights=weights, ind_k=ind_group[[k]], z=z, mu_pres=mu0, sigma_pres=sigma0, q=quadrature, lower=-5, upper=5)
            phi1[j,2+k]=par_updates_g$par
          }
        }
      }
    ...
    for (k in 1:p){
      par_updates=optim(par=sigma0[k], fn=target_var, gr=grad_var, method="L-BFGS-B",
        k=k, x=x, z=z, weights=weights, mu_pres=mu0, sigma_pres=sigma0, q=quadrature, lower=0.1, upper=5)
      sigma1[k]=par_updates$par
    }
    MLL=mml(a=phi1[,1], b=phi1[,2], g=phi1[,c(3:(2+p))], x, dat, weights, z, mu=mu1, sigma=sigma1, w)
    message("  MLL = ", round(MLL, 2))
  }
...
# Use their published a_ini as starting discriminations
...
message("Starting EM (tol=0.05, ite=1)...")
