...
2. Please check that the `id` and `item` identifiers have an appropriate number of unique values.
...
5. Before sending a file, check it with our validator, the same checks we run before a table is added to the IRW. The quickest way is to drop the file on the [browser validator](validate.qmd): it needs nothing installed, and your file stays on your computer. From the command line it needs Python but no account or credentials:
    ...
    Both report two things: whether the file conforms to [the IRW Data Standard](standard.qmd), and whether it passes the IRW's own intake checks, such as the minimum of 100 respondents. Each problem is an error (must be fixed) or a warning (worth a look; some are legitimate). R users can use the browser validator, or run a smaller set of core checks with `source("https://raw.githubusercontent.com/ben-domingue/irw/main/misc/validate_irw.R")` followed by `validate_irw(df, "mydata.csv")`.
