...
5. Before sending a file, check it with our validator, the same checks we run before a table is added to the IRW. It needs Python but no account or credentials:
    ...
    It reports each problem as an error (must be fixed) or a warning (worth a look; some are legitimate). R users can run a smaller set of core checks with `source("https://raw.githubusercontent.com/ben-domingue/irw/main/misc/validate_irw.R")` followed by `validate_irw(df, "mydata.csv")`.
