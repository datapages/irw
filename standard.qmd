...
- `id` This is a persistent identifer for the focal unit, i.e. *the thing being measured*. This wlil typically be a person but in some cases may be, for example, a word (when interest is in establishing some property of the word).
...
- `wave/observation` In settings wherein data is collected longitudinally (but precise timing as in `date` is not available), we indicate the wave of data collection. Note that this is also used to indicate pre/post treatment in the case of data collected from randomized controlled trials (RCTs).
...
In some cases, we deviate from the above rules for specific reasons. We describe those here. Some of the below may be considered more experimental features of the IRW.

- Process data: In general, a single row of data in the IRW corresponds to a unque response from an individual to an item. In the context of ['process data'](https://link.springer.com/article/10.1007/s11336-022-09880-8), more information about a response is available. [example]

- Trials: Many constructs in cognitive psychology are measured via repeated trials of similar tasks wherein the probes either do not vary or vary in terms of some quantifiable feature of the stimulus. In such cases, [what do we do?]
...
4. If there are multiple scales available, the responses need to be split into mutiple files (one per scale).  
