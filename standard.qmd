...
- `wave/observation` In settings wherein data is collected longitudinally (but precise timing as in `date` is not available), we indicate the wave of data collection. Note that this is also used to indicate pre/post treatment in the case ofdata collected from randomized controlled trials (RCTs).
...
perhaps outline key issues that we will analyze in deposition process?
