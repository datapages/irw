...
This year’s competition centers on predicting cognitive performance using data from the Hearts and Flowers (H&F) task, a widely used measure of executive functioning. For more information on this task from the group that collected these data, see [here](https://sparklab.stanford.edu/ames).  These data were collected from a large urban district in the US. In particular, these data were collected in school environments. Students completed the Hearts and Flowers measures on digital devices during the school day.
...
* `time_limit`: Time limit for the block (response still collected after the time limit but respondents know they have exceed the time limit).
...
* Response time is potentially a valuable predictor of response accuracy. In particular, note that respondents may enter responses after the time limit in some variants of the task but they are focused on entering responses prior to the time limit. 

### Judging criteria

We note two criteria that will be used in judging this competition. Some attention will be paid to accuracy in the test dataset. However, there will also be a subjective evaluation of the degree to which the predictive approach is novel or forwards new insights about the nature of the response process. 

### Submission
Submission instructions are available [here](https://www.psychometricsociety.org/imps2025-datathon-format). Submissions can be made [here](https://forms.gle/3GRpyBq9jdWUW7cS8).
