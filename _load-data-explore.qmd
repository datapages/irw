...
## `Not applicable (non-person)` replaced `Non-human` (irw#2206). Both stay
## listed so the filter orders either spelling while the published tags catch up.
age_range_vals <- c("Child (<18y)", "Adult (18+)", "Elderly (minimum age >50)", "Mixed",
                    "Not applicable (non-person)", "Non-human", "NA")
child_age_vals <- c("Early (<6y)", "Child (6-12y)", "Adolescent (12-18y)", "NA")
has_item_text_vals <- c("Yes", "No")

sort_funs <- list(
  ## Known values in their fixed order, then any value the list does not know
  ## yet -- a hard-coded list alone silently drops a new value from the filter.
  "age_range" = \(v) c(setdiff(intersect(age_range_vals, v), "NA"),
                        setdiff(v, age_range_vals), intersect("NA", v)),
...
## Shard name -> Redivis scoped reference, for the embed URLs in "Look up a
## single dataset". Authoritative source is IRW_CORE_DATASETS in
## ben-domingue/irw metadata/redivis_config.R; landing/page_rules.R mirrors it.
ds_ref <- c(
...
## One listing of every shard, shared by the explorer (urls) and the lookup
## (table_ds_ref). The lookup used to live in _load-data.qmd, which listed all
## six shards and fetched biblio a second time on every render (irw#1719, 17.4).
listing <- names(ds_ref) |>
  map(\(d) redivis$user("datapages")$dataset(d)$list_tables() |>
        map(\(t) tibble(table = t$name, url = t$properties$url, ds_ref = ds_ref[[d]])) |>
        list_rbind()) |>
  list_rbind() |>
  mutate(table = str_to_lower(table))
urls <- listing |> select(table, url) |> mutate(url = str_remove(url, "\\?.*$"))
ojs_define(table_ds_ref = listing |> select(table, ds_ref))

## The data dictionary for the lookup, from the same biblio fetch as above.
data_index <- biblio |>
