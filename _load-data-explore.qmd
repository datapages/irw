...
# table_comp <- list(
#   metadata_table |> transmute(table = str_to_lower(table)) |> mutate(in_metadata = TRUE),
#   tag_table |> transmute(table = str_to_lower(table)) |> mutate(in_tags = TRUE),
#   biblio |> transmute(table = str_to_lower(table)) |> mutate(in_biblio = TRUE)
# ) |> reduce(full_join) |>
#   mutate(across(everything(), \(s) replace_na(s, FALSE)))

# table_comp |> filter(!in_metadata | !in_tags | !in_biblio) |> View()
# table_comp |> count(in_metadata, in_tags, in_biblio, name = "n_tables")
# table_comp |> filter(in_tags, )
