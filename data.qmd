...
# this grows with it and no OJS changes. The emitter runs post-render and cannot
# tell this chunk anything, so the shared file is the contract.
ojs_define(pilot_tables = local({
...
Selected tables also have their own citable page, listed at [IRW table pages](/tables/). Each names the IRW version it describes and carries `schema.org/Dataset` and Croissant metadata. This is a pilot covering 25 tables ([irw#1706](https://github.com/ben-domingue/irw/issues/1706)); the browser above remains the way to search the whole warehouse.
