...
- [**Browse table pages**](/tables/) — every table with a recorded licence has its own page, giving a description of what it measures and where it came from, its size and shape (responses, respondents, items and response categories), the tags that classify it, item text statistics where they exist, the columns it contains, and ready-to-run R and Python code to fetch it. Each page names the IRW version it describes, so it can be cited.
...
#| echo: false
#| message: false
# Which tables have a landing page under /tables/ (ben-domingue/irw#1706). The
# emitter runs post-render and cannot tell this page anything, so both read the
# same rule from landing/page_rules.R: a table gets a page unless it has no
# recorded licence (held, see irw#2266). Withdrawn tables have a tombstone, not a
# page, and are not linked from here.
source(file.path("landing", "page_rules.R"))
ojs_define(page_tables = tolower(page_tables(metadata_table, biblio)))
```
...
has_page = new Set(
  (typeof page_tables === "undefined" || page_tables === null
     ? []
     : (Array.isArray(page_tables) ? page_tables : [page_tables])
...
// and $text is a URL. data_url also holds values like "NA" and "Author Permission";
// linked, those became relative hrefs that crawlers followed to 404s.
pullout = (head, text) => html.fragment`<p><b>${head}</b>: ${/Link/.test(head) && /^https?:\/\//.test(String(text)) ? html.fragment`<a href="${text}" target="_blank">${text}</a>` : text}</p>`
...
row ? html`<div class="pullout">
  ...
  ${has_page.has(String(row.table).toLowerCase())
      ? html.fragment`<p><b>Citable page</b>: <a href="${table_page_href(row.table)}">${table_page_href(row.table)}</a> &mdash; a permanent page for this table, naming the IRW version it describes.</p>`
      : html.fragment``}
...
Below we show an example dataset and illustrations of how to access the data programmatically. You can also explore the data [here](https://redivis.com/datasets/as2e-cv7jb41fd/tables).
