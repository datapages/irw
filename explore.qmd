...
vi = ({
  ...
  "density":                   { label: "Density (density)",                                     base: 10 }
})
...
::: {.callout-note title="Information on selected table"}
```{ojs}
info = ({
  "table":       { label: "Dataset"        },
...
</div>` : html`<div class="pullout"><i>${placeholder}</i></div>`
```
:::
...
::: {.callout-note collapse="true" title="Longitudinality"}
