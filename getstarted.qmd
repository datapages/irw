...
# Install package with `pip install irw`
import irw
...
## Option 4: Work with an AI Assistant

If you use an AI assistant such as Claude, ChatGPT, or Gemini, you can point it at the IRW in one of two ways. You can paste a short briefing that teaches it the data standard and the defaults that go wrong silently. If your assistant supports MCP, you can also install the IRW server (`pip install "irw[mcp]"`), which lets it search the warehouse and read tables using your own Redivis account. See [Using an AI Assistant with the IRW](agents.qmd) for both.
