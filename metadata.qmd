...
We supplement the data in the IRW tables with various kinds of metadata about the tables. For clarity, we separatley describe the qualitative and quantitative metadata. 
...
- 'responses_per_participant' The averague number of responses for an object of measurement.
...
Given the volume of tables in the IRW and their heterogeneity, being able to effectively query IRW tables is essential. To do so, we recommend using the `irw::irw_filter()` function. Information on that function is [here](https://itemresponsewarehouse.github.io/Rpkg/reference/irw_filter.html). Below we provide a variety of simple use cases.
